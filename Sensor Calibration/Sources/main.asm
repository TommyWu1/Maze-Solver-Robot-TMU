...
CR          EQU $0D	; Carriage Return (0x0D)
SPACE       EQU ' '	; Space character (ASCII 0x20)
...
_Startup:
            ...
            JSR CLR_LCD_BUF	; Write space characters (ASCII 0x20) to the LCD buffer

;---------------------------------------------------------------------------
; Display Sensors

MAIN        JSR G_LEDS_ON	; Enable guider LEDs
            JSR READ_SENSORS	; Read the five guider sensors
...
INIT        BCLR DDRAD,$FF	; Make PORTAD an input (DDRAD @ $0272)
            ...
            BSET DDRJ,$C0	; Make PTJ pins 7 and 6 outputs (DDRJ @ $026A)
            RTS
...
openADC     MOVB #$80,ATDCTL2	; Turn on ADC (ATDCTL2 @ $0082)
            ...
            JSR del_50us	; Delay helper (~50 us)
            MOVB #$20,ATDCTL3	; 4 conversions on AN1 (ATDCTL3 @ $0083)
...
; Passed: X = source address (start of null-terminated string)
...
STRCPY_LOOP LDAA 0,X	; Get source character
            STAA 0,Y	; Copy it to the destination
            BEQ STRCPY_EXIT	; If null, then exit
...
; Passed: none
; Returns: none
; Side effects: PORTA bit 5 changed

G_LEDS_ON   BSET PORTA,%00100000	; Set guider LED bit (bit 5)
            RTS
...
G_LEDS_OFF  BCLR PORTA,%00100000	; Clear guider LED bit (bit 5)
            RTS
...
RS_MAIN_LOOP LDAA SENSOR_NUM	; Select sensor input index
            JSR SELECT_SENSOR	; Enable sensor on hardware
            LDY #400		; ~20 ms delay to allow sensor to stabilize
            JSR del_50us	; Delay helper

            LDAA #%10000001	; Start A/D conversion on AN1
            STAA ATDCTL5
            BRCLR ATDSTAT0,$80,*	; Wait until conversion complete

            LDAA ATDDR0L	; Read A/D result from ATDDR0L
            STAA 0,X		; Store into sensor array
            CPX #SENSOR_STBD	; If this is the last sensor
            BEQ RS_EXIT		; then exit loop

            INC SENSOR_NUM		; Increment sensor index
            INX		; Advance sensor array pointer
            BRA RS_MAIN_LOOP	; Repeat for next sensor
...
SELECT_SENSOR PSHA	; Save the sensor number on the stack

            LDAA PORTA	; Read PORTA
            ANDA #%11100011	; Clear sensor-select bits (PA2..PA4)
            STAA TEMP	; Save masked PORTA to TEMP

            PULA		; Retrieve sensor number from stack
            ASLA		; Shift left once
            ASLA		; Shift left twice (align to PA2..PA4)
            ANDA #%00011100	; Mask to sensor-select bits

            ORAA TEMP	; Merge with preserved PORTA bits
            STAA PORTA	; Write updated PORTA to hardware
            RTS
...
; Returns: none
; Side effects: modifies display buffer and related state
...
DISPLAY_SENSORS LDAA SENSOR_BOW	; Get front sensor value
            JSR BIN2ASC	; Convert to ASCII (2 chars) in D
            LDX #DP_FRONT_SENSOR	; LCD buffer position for front sensor
            STD 0,X		; Store two ASCII characters

            LDAA SENSOR_PORT	; Port sensor
            JSR BIN2ASC
            LDX #DP_PORT_SENSOR
            STD 0,X
            ...
            LDAA SENSOR_STBD	; Starboard sensor
            JSR BIN2ASC
            LDX #DP_STBD_SENSOR
            STD 0,X

            LDAA SENSOR_LINE	; Line/differential sensor
            JSR BIN2ASC
            LDX #DP_LINE_SENSOR
            STD 0,X

            LDAA #CLEAR_HOME	; Clear display and home cursor
            JSR cmd2LCD	; Send clear-home command

            LDY #40		; Wait ~2 ms for clear to complete
            JSR del_50us	; Delay helper

            LDX #TOP_LINE	; Copy top-line buffer to LCD
            JSR putsLCD

            LDAA #LCD_SEC_LINE	; Position cursor on second line
            JSR LCD_POS_CRSR

            LDX #BOT_LINE	; Copy bottom-line buffer to LCD
            JSR putsLCD
            RTS
...
BIN2ASC     PSHA		; Save a copy of input number on the stack
            TAB		; Copy to ACCB
            ANDB #%00001111	; Mask lower nibble of ACCB
            CLRA		; D now contains 000n where n is the LSnibble
            ADDD #HEX_TABLE	; Set up for indexed load
            XGDX
            LDAA 0,X	; Get LSnibble character

            PULB		; Retrieve original input into ACCB
            PSHA		; Push LSnibble character back on stack
            RORB		; Rotate right to move upper nibble down
            RORB		; (repeat)
            RORB
            RORB
            ANDB #%00001111	; Mask lower nibble
            CLRA		; D now contains 000n where n is MSnibble
            ADDD #HEX_TABLE	; Set up for indexed load
            XGDX
            LDAA 0,X	; Get MSnibble character into ACCA
            PULB		; Retrieve LSnibble character into ACCB
            RTS
...
openLCD     LDY #2000	; Wait ~100 ms for LCD to become ready
            JSR del_50us	; Delay helper
            LDAA #INTERFACE	; Set 8-bit data, 2-line display, 5x8 font
            JSR cmd2LCD	; Send command
            LDAA #CURSOR_OFF	; Display on, cursor off
            JSR cmd2LCD	; Send command
            LDAA #SHIFT_OFF	; Move cursor right (address increments)
            JSR cmd2LCD	; Send command
            LDAA #CLEAR_HOME	; Clear display and home cursor
            JSR cmd2LCD	; Send command
            LDY #40	; Wait ~2 ms for clear to complete
...
cmd2LCD     BCLR LCD_CNTR,LCD_RS	; Select LCD instruction register
            JSR dataMov	; Send data to IR/DR
            RTS
...
putsLCD     LDAA 1,X+	; Get next character from string
            BEQ donePS	; If NULL, we're done
            JSR putcLCD
            BRA putsLCD
...
dataMov     BSET LCD_CNTR,LCD_E	; Pull LCD E signal high
            STAA LCD_DAT	; Send 8 data bits to LCD
            NOP
            NOP
            NOP
            BCLR LCD_CNTR,LCD_E	; Pull E low to complete write

            LDY #1		; Short delay to allow internal operations
            JSR del_50us	; Delay helper
            RTS
...
LCD_POS_CRSR ORAA #%10000000	; Set high bit of control word
            JSR cmd2LCD	; Send cursor-position command
            RTS
...
eloop       LDX #300	; (2 E-clk) Inner loop counter
iloop       NOP		; (1 E-clk) No operation

            DBNE X,iloop	; (3 E-clk) Loop inner counter
            DBNE Y,eloop	; (3 E-clk) Loop outer counter
