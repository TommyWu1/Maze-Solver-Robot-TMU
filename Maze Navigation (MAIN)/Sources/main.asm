...
CR            EQU   $0D                   ; Carriage Return character (0x0D)
...
LINE_VARIANCE           FCB   $18           ; Variance values determined from testing
BOW_VARIANCE            FCB   $30           ; Baseline offset for bow sensor
...
NO_BLANK      ds.b  1                       ; Used for leading-zero blanking by BCD2ASC
...
;---------------------------------------------------------------------------
;                           Clear LCD Buffer
; This routine writes space characters (ASCII 0x20) into the LCD display
; buffer to initialize it. It only needs to run once at startup; the display
; update routine maintains the buffer during normal operation.
...
; -------------------------------------------------------------------------------------------------
; String Copy
; Copy a null-terminated string (including the null) from source to dest.
...
; -------------------------------------------------------------------------------------------------
;                                   Guider LEDs ON
; Enable the guider LEDs so sensor readings reflect the illuminated condition.
; Passed: none
; Returns: none
; Side effect: PORTA bit 5 is set
G_LEDS_ON         BSET PORTA,%00100000 ; Set guider LED bit (bit 5)
                  RTS

; -------------------------------------------------------------------------------------------------
;                                   Guider LEDs OFF
; Disable the guider LEDs so sensor readings reflect ambient light.
; Passed: none
; Returns: none
; Side effect: PORTA bit 5 is cleared
...
RS_MAIN_LOOP      LDAA  SENSOR_NUM     ; Select the correct sensor input
                  ...
                  LDY   #200           ; 10 ms delay to allow the
...
UPDT_DISPL        MOVB    #$90,ATDCTL5    ; R-just., uns., sing. conv., mult., ch=0, start
                  ...
                  JSR     putcLCD         ; same for THOUSANDS, decimal point and HUNDREDS
                  LDAA    #$C7            ; Move LCD cursor to the 2nd row, end of msg2
                  JSR     cmd2LCD         ;
                  LDAB    CRNT_STATE      ; Load current state index
                  LSLB                    ; Multiply index by 2 (shift left)
                  LSLB
                  LSLB
                  LDX     #tab            ; Point to state string table
                  ABX                     ; Add index to table pointer
                  JSR     putsLCD         ; Display the state string
                  RTS
...
; Convert the BCD number in the BCD buffer into ASCII with leading-zero
; suppression. Leading zeros are converted into space characters. The
; NO_BLANK flag is cleared initially and set once a non-zero digit is
; encountered. The units digit is never blanked.
BCD2ASC           LDAA    #0            ; Initialize the blanking flag
                  STAA    NO_BLANK

C_TTHOU           LDAA    TEN_THOUS     ; Check ten-thousands digit
                  ORAA    NO_BLANK
                  BNE     NOT_BLANK1
...
NOT_BLANK1        LDAA    TEN_THOUS     ; Get the ten-thousands digit
                  ...
                  STAA    TEN_THOUS
                  LDAA    #$1           ; Set NO_BLANK to indicate a non-blank digit
                  STAA    NO_BLANK
...
NOT_BLANK2        LDAA    THOUSANDS     ; Convert to ASCII like ten-thousands case
                  ORAA    #$30
                  STAA    THOUSANDS
                  LDAA    #$1
                  STAA    NO_BLANK
...
C_HUNS            LDAA    HUNDREDS      ; Check the hundreds digit for blankness
                  ORAA    NO_BLANK      ; If it's blank and NO_BLANK is still zero
                  BNE     NOT_BLANK3
...
NOT_BLANK3        LDAA    HUNDREDS          ; Convert to ASCII (similar to ten-thousands case)
                  ORAA    #$30
                  STAA    HUNDREDS
                  LDAA    #$1
                  STAA    NO_BLANK
...
NOT_BLANK4        LDAA    TENS          ; Convert to ASCII (similar to ten-thousands case)
                  ORAA    #$30
                  STAA    TENS
